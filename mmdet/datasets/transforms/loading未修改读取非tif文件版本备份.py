...
class LoadFusionDataFromFiles(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            ...
            if (self.suffix == None) and (self.if_fusion==False):
                img = mmcv.imfrombytes(img_bytes, flag=self.color_type, backend=self.imdecode_backend)

            # 有suffix且不是融合数据, 仅读取NIR图像
            elif (self.suffix is not None) and (self.if_fusion==False):
                img = tiff.imread(filename[:-3] + self.suffix)

            # 融合数据, 读取RGB和NIR图像并concat
            elif (self.suffix is not None) and self.if_fusion:
                img = mmcv.imfrombytes(img_bytes, flag=self.color_type, backend=self.imdecode_backend)
                spec = tiff.imread(filename[:-3] + self.suffix)
