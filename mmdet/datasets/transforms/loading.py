...
class LoadFusionDataFromFiles(BaseTransform):
    ...
    def __init__(self,
                 ...
                 suffix: str = None,  # 新增读取光谱图像的后缀
                 if_fusion: bool = False,  # 是否读取多特征图像
                 *,
    ...
    # 独立出此部分以美化代码
    def client_(self, filename: str) -> bytes:  # 后缀不确定
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename = results['img_path']  # 单个图像绝对路径

        try:
            # 不是融合数据时:
            if self.if_fusion==False:
                # 只要不为tif文件, 都仅当单模态RGB图像读取
                if self.suffix != 'tif':
                    img_bytes = self.client_(filename)
                    img = mmcv.imfrombytes(img_bytes, flag=self.color_type, backend=self.imdecode_backend)
                # tif数据, 仅读取单模态NIR图像
                else:  # self.suffix == 'tif'
                    spec_ = tiff.imread(filename[:-3] + self.suffix)
                    img = (spec_ / 15.6863).round().astype(np.uint8)  # (4000/255=15.6863)
            # 融合数据时:
            else:  # self.if_fusion==True
                # 融合数据, 读取RGB和NIR图像并concat
                if self.suffix != 'tif':
                    img_bytes = self.client_(filename)
                    img_ = mmcv.imfrombytes(img_bytes, flag=self.color_type, backend=self.imdecode_backend)
                    spec_bytes = self.client_(filename[:-3] + self.suffix)
                    spec_ = mmcv.imfrombytes(spec_bytes, flag=self.color_type, backend=self.imdecode_backend)
                    # concat
...
@TRANSFORMS.register_module()
class LoadImageFromNDArray(LoadFusionDataFromFiles):
