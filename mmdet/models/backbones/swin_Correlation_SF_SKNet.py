...
# 使用原版SKNet分别处理四个模态, RGB, HSI, R_H, H_R
@MODELS.register_module()
class Corr_SF_SKNet_SwinTransformer(BaseModule):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(Corr_SF_SKNet_SwinTransformer, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(Corr_SF_SKNet_SwinTransformer, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        for i in range(len(outs_RGB)):
            concat_out=(torch.concat((outs_RGB[i],
                                      ...
                                      outs_H_R[i]), dim=1))
            # 先经过SFNet
            SK_temp = self.divide_sk_block[i](concat_out)
            # 再经过SKNet
            SK_out = self.sk_block[i](SK_temp)
