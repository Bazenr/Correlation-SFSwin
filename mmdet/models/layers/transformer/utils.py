...
class PatchEmbed(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tuple[int]]:
        ...
        # projection:  [B, C, H, W] -> [B, embed_dims(C'), H/4, W/4]
        out_size = (x.shape[2], x.shape[3])
        # flatten: [B, C', H/4, W/4] -> [B, C, H/4*W/4]
        # transpose: [B, C', H/4*W/4] -> [B, H/4*W/4, C']
...
class CorrelationEmbed(BaseModule):
    """Image to Patch Embedding.
    ...
    def __init__(self,
                 ...
                 norm_cfg: OptConfigType = None,
                 input_size: Union[int, tuple] = None,
                 if_RGB_q: bool = False,  # False就选第一个模态, True就第二个, 其他同理
                 if_RGB_k: bool = False,  # 将不同模态的q, k, v重新排布
                 if_RGB_v: bool = False,  # 
                 init_cfg: OptConfigType = None) -> None:
        super(CorrelationEmbed, self).__init__(init_cfg=init_cfg)

        self.if_RGB_q = if_RGB_q  # 
        self.if_RGB_k = if_RGB_k  # 
        self.if_RGB_v = if_RGB_v  # 
        ...
        self.projection = build_conv_layer(
            ...
            bias=bias)
        # #
        self.projection_HSI = build_conv_layer(
            dict(type=conv_type),
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tuple[Tensor, Tuple[int]]:
        """
        Args:
            x (Tensor): Has shape (B, C, H, W). In most case, C is 3.
            y (Tensor): Has shape (B, C', H, W). In most case, C': bands.
        ...
        if self.adap_padding:
            x = self.adap_padding(x)
            y = self.adap_padding(y)
        x = self.projection(x)
        y = self.projection_HSI(y)  # 投影
        q_dim = int(self.embed_dims / 3)
        # #
        if self.if_RGB_q:           x[:, 0:q_dim, :, :] = y[:, 0:q_dim, :, :]
        if self.if_RGB_k:     x[:, q_dim:q_dim*2, :, :] = y[:, q_dim:q_dim*2, :, :]
        if self.if_RGB_v:   x[:, q_dim*2:q_dim*3, :, :] = y[:, q_dim*2:q_dim*3, :, :]
        # projection:  [B, C, H, W] -> [B, embed_dims(C'), H/4, W/4]
        # #
        out_size = (x.shape[2], x.shape[3])
        # flatten: [B, C', H/4, W/4] -> [B, C, H/4*W/4]
...
# class FusionCorrelationEmbed(BaseModule):
#     """Image to Patch Embedding.
...
class PatchMerging(BaseModule):
    ...
    def forward(self, x: Tensor,
        ...
        # view & permute:  [B, HW, C] -> [B, H, W, C] -> [B, C, H, W]

        if self.adap_padding:
            x = self.adap_padding(x)
            H, W = x.shape[-2:]

        x = self.sampler(x)  # nn.Unfold
        # if kernel_size=2 and stride=2, x should has shape (B, 4*C, H/2*W/2)
        # sampler取样:  [B, C, H, W] -> [B, 4C, H/2*W/2]
        ...
        # transpose:  [B, 4C, H/2*W/2] -> [B, H/2*W/2, 4C]
        x = self.norm(x) if self.norm else x
        x = self.reduction(x)
        # transpose:  [B, H/2*W/2, 4C] -> [B, H/2*W/2, 2C]
