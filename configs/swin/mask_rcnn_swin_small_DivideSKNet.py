...
load_from = R'H:\projects\mmdetr_rebuild\pretrain_model/' + \
              'double_swin_rebuild/pretrain_double_swin_rebuild.pth'
...
fpn_channels = [192, 384, 768, 1536]  # [96, 192, 384, 768]
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DivideSKNet_DoubleSwinTransformer',  # 
        in_chans1=3,  # 
        in_chans2=3,  # 
        embed_dims=96,
        # # 新版SKNet参数
        SK_channels = fpn_channels,
        M = 2,   # Split步骤的分支数, # 双主干时使用2
        #  WH = ?,  # 图像长宽
        G = 32,  # Split步骤卷积的分组数, 32
        r = 8,   # Fuse步骤FC层的缩减率, 16
        L = 32,  # 原文默认32
        #  decrease [3, 2, 1, 0], increase [0, 1, 2, 3]
        conv_scales = [0, 1, 2, 3],  # SKNet特征融合用不同卷积层尺度
        # # 
        # 与之前的区别1
        depths=[2, 2, 6, 2],  # 6, 18
        num_heads=[3, 6, 12, 24],
