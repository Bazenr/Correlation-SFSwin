...
model_cfg_name = "mask_rcnn_swin_small_Correlation_DivideSKNet" + ".py"
# mask-rcnn_swin-tiny-to-small, mask-rcnn_swin-t-p4-w7_fpn_1x_coco

out_name = "One_step"
# One step

out_fig = R"H:\projects\mmdetr_rebuild\work_dirs/" + out_name + ".png"
out_doc =  R"H:\projects\mmdetr_rebuild\work_dirs/" + out_name + ".txt"
...
def parse_args():
    ...
    parser.add_argument('-p', '--parameter', type=str, choices=['lr', 'momentum', 'wd'],
        default='lr', help='The parameter to visualize its change curve, choose from'
                           '"lr", "wd" and "momentum". Defaults to "lr".')
    parser.add_argument('--title', type=str, help='title of figure',
        default=None)
    parser.add_argument('-s', '--save-path', type=Path, help='The learning rate curve plot save path',
        default=out_fig)
    parser.add_argument('-d', '--dataset-size', type=int,
            help='The size of the dataset. If specify, `build_dataset` will '
            'be skipped and use this size as the dataset size.')
    parser.add_argument('-n', '--ngpus', type=int, help='The number of GPUs used in training.',
        default=1)
    parser.add_argument('--log-level',
        default='WARNING', help='The log level of the handler and logger. Defaults to WARNING.')
    parser.add_argument('--style', type=str, help='style of the figure, need `seaborn` package.',
        default='whitegrid')
    parser.add_argument('--not-show',action='store_true',
        default=False)
    parser.add_argument('--window-size', help='Size of the window to display images, in format of "$W*$H".',
        default='12*7')
...
def plot_curve(lr_list, args, param_name, iters_per_epoch, by_epoch=True):
    ...
    plt.figure(figsize=(wind_w, wind_h), dpi=200)
    # print(lr_list)
    ...
    for i, lr in enumerate(lr_list):
        file.write("epoch_"+ str(i) + ":\t" + str(lr) + "\n")
    file.close()
    ''''''
...
def main():
    ...
    #     dataset_size = len(DATASETS.build(cfg.test_dataloader.dataset))
