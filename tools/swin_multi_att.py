...
'''双主干Swin'''
@MODELS.register_module()
class Double_SwinTransformer(nn.Module):
    r"""Double Backbone(Swin Transformer)
        For RGB-HSI fusion dataset.
    """
    def __init__(self,
                 patch_size=4,  # patch partition的倍率
                 in_chans1=3,
                 in_chans2=224,
                 embed_dims=96,  # Linear embedding后的C
                 depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24),  # 看原论文
                 window_size=7, 
        ...
        self.swin_RGB = SwinTransformer(patch_size=patch_size,
        ...
        self.swin_HSI = SwinTransformer(patch_size=patch_size,
    ...
    def forward(self, x):
        ...
        out_rgb = self.swin_RGB(x1)
        out_hsi = self.swin_HSI(x2)
        # input: [B, 6, 32, 32]
        # out_rgb.shape: [B, 96, 8, 8], [B, 192, 4, 4],
        #                [B, 384, 2, 2], [B, 768, 1, 1]
        concat_outs = []
...
class SKConv(nn.Module):
    ...
    def forward(self, x):
        ...
        vectors = softmax_att.unsqueeze(-1).unsqueeze(-1)
...
# SKNet融合的双主干Swin
@MODELS.register_module()
class SKNet_SwinTransformer(nn.Module):
    r"""Double Backbone(Swin Transformer)
        For RGB-HSI fusion dataset.
    """
    def __init__(self,
    ...
    def forward(self, x):
        ...
        for i in range(len(out_rgb)):
            ...
            # print(' SK_out.shape', SK_out.shape)
            # print('*********************')
